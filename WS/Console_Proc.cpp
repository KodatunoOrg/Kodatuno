	...
	switch(keycode){
		...
		case WSK_Return:
			...
			if(newlen == oldlen)	break;					// 何も入力されていなかったら、何もしない
				...
				Kodatuno.SetConsoleHistory(mes);			// コマンドを履歴に登録
				//Kodatuno.SetConsoleCharNum(newlen);			// コンソール文字列長を更新
			}
			break;
		// 上下キーに対してはコマンド履歴を表示する
		case WSK_KP_Up:
			if(Kodatuno.GetUpperConsoleHistory(&com) == KOD_TRUE){	// 新たな履歴コマンドを取得
				...
				SetCommand((char *)com);							// 新しい履歴コマンドをコンソールに出力
			}
			key->setKey(0);
			break;
		case WSK_KP_Down:
			if(Kodatuno.GetLowerConsoleHistory(&com) == KOD_TRUE){
