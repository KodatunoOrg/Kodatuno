...
int IGES_PARSER::GetNurbsCPara(char str[],int pD,DirectoryParam *dpara,BODY body)
{
	...
	// メモリー確保
	if(hbody.New_NurbsC(&body.NurbsC[TypeCount[_NURBSC]],body.NurbsC[TypeCount[_NURBSC]].K,body.NurbsC[TypeCount[_NURBSC]].N) == KOD_ERR){
	...
	// ノットベクトルの範囲が0～1でなかった場合は、強制的に0～1にする
	//CheckKnotVecRange(body.NurbsC[TypeCount[_NURBSC]].V,
	//	body.NurbsC[TypeCount[_NURBSC]].T,
	//	body.NurbsC[TypeCount[_NURBSC]].N,
	//	body.NurbsC[TypeCount[_NURBSC]].M,
	//	body.NurbsC[TypeCount[_NURBSC]].K);	
...
int IGES_PARSER::GetNurbsSPara(char str[],int pD,DirectoryParam *dpara,BODY body)
{
	...
	// メモリー確保
	if(hbody.New_NurbsS(&body.NurbsS[TypeCount[_NURBSS]],body.NurbsS[TypeCount[_NURBSS]].K,body.NurbsS[TypeCount[_NURBSS]].N) == KOD_ERR){
		SetMessage("PARAMETER SECTION KOD_ERROR:fail to allocate memory");
		return KOD_ERR;
	}
	...
	// ノットベクトルの範囲が0～1でなかった場合は、強制的に0～1にする
	//CheckKnotVecRange(body.NurbsS[TypeCount[_NURBSS]].U,
	//	body.NurbsS[TypeCount[_NURBSS]].S,
	//	body.NurbsS[TypeCount[_NURBSS]].N[0],
	//	body.NurbsS[TypeCount[_NURBSS]].M[0],
	//	body.NurbsS[TypeCount[_NURBSS]].K[0]);
	//CheckKnotVecRange(body.NurbsS[TypeCount[_NURBSS]].V,
	//	body.NurbsS[TypeCount[_NURBSS]].T,
	//	body.NurbsS[TypeCount[_NURBSS]].N[1],
	//	body.NurbsS[TypeCount[_NURBSS]].M[1],
	//	body.NurbsS[TypeCount[_NURBSS]].K[1]);
...
// ノットベクトルの範囲が0～1でなかった場合は、強制的に0～1にする
int IGES_PARSER::CheckKnotVecRange(double Range[2],double Knot[],int N,int M,int K)
{
	if(Range[0] != 0.0 || Range[1] != 1.0){		// パラメータ範囲が0～1でなかった場合は、強制的に0～1にする
		double _t[KNOTNUMMAX];
		for(int i=0;i<N;i++){
			_t[i] = (Knot[i] - Knot[M-1]) / (Knot[K] - Knot[M-1]);
		}
		for(int i=0;i<N;i++){
			Knot[i] = _t[i];
			if(i >= M && i <= N-M && Knot[i] < 0.001)	// ノットベクトルの値が小さい場合、OpenGL側でゼロと判断してしまうため、そうならない値を便宜的に代入
				Knot[i] = 0.001;
		}
	}
	Range[0] = 0.0;
...
int IGES_PARSER::GetCompCPara(char str[],int pD,DirectoryParam *dpara,int dline,BODY body)
{
	...
	// 複合曲線のメモリーを確保
	if(hbody.New_CompC(&body.CompC[TypeCount[_COMPOSITE_CURVE]],body.CompC[TypeCount[_COMPOSITE_CURVE]].N) == KOD_ERR){
		SetMessage("PARAMETER SECTION KOD_ERROR:fail to allocate memory");
		return KOD_ERR;
	}
...
int IGES_PARSER::GetTrmSPara(char str[],int pD,DirectoryParam *dpara,BODY body)
{
	...
	// 単純閉曲線N2の数だけメモリー確保
	if((hbody.New_TrmS(&body.TrmS[TypeCount[_TRIMMED_SURFACE]],body.TrmS[TypeCount[_TRIMMED_SURFACE]].n2)) == KOD_ERR){
		SetMessage("PARAMETER SECTION KOD_ERROR:fail to allocate memory");
		return KOD_ERR;
	}
