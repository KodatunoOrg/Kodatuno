...
public:
	...
	void Draw_NurbsCurves(BODY *Body);			// BODYに含まれるNURBS曲線を全て描画
	void Draw_NurbsSurfaces(BODY *Body);		// BODYに含まれるNURBS曲面を全て描画
	void Draw_TrimSurfes(BODY *Body);			// BODYに含まれるトリム面を全て描画
	...
	void DrawMesh(MESH *,int);					// メッシュの描画
