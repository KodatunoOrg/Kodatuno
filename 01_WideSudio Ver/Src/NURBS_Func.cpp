...
int NURBS_Func::GenIsoparamCurveV(NURBSS *P,double v,NURBSC *C)
{
	...
	double U[2] = {P->U[0],P->U[1]};	// ノットベクトルの範囲
	int prop[4] = {0,0,1,0};		// パラメータ

	Coord Q[P->K[0]];	// コントロールポイント
	double W[P->K[0]];	// ウェイト
	...
	GenNurbsC(C,P->K[0],P->M[0],P->N[0],P->S,W,Q,U,prop,0);
...
Coord NURBS_Func::CalcAveCurvatureNormVec(NURBSS *nurb,double u,double v)
{
	...
	Coord K = MulCoord(n,-(G*L+E*N-2*F*M)/(E*G-F*F)/2);	// 平均曲率ベクトル
...
int NURBS_Func::CalcIntersecPtsPlaneSearch(NURBSS *nurb,Coord pt,Coord nvec,double ds,int initdivnum,Coord *ans,int ans_size)
{
	...
	if(!init_pt_num)
		init_pt_num = CalcIntersecPtsPlaneU3(nurb,pt,nvec,100,init_pt,INTERSECPTNUMMAX);		// それでも交点が見つからなかった場合は方向を変えてサーチ
...
int NURBS_Func::TrimNurbsSPlane(TRMS *Trm,Coord pt,Coord nvec)
{
	...
	num = CalcIntersecPtsPlaneSearch(Trm->pts,pt,nvec,0.5,5,t,2000);		// NURBS曲面と平面との交点群を交線追跡法で求める
