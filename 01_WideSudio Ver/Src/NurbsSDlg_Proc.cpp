...
void NurbsSDlg_Proc(WSCbase* object)
{
	...
	if(NurbsSurface_Dlg->getStatus() == WS_DIALOG_OK){		// OKボタンが押された
		...
		switch(val){
			case 1:
				SetMessage("You chose polygonal NURBS surface generation method.");
				break;
			case 2:
				SetMessage("You chose interpolated NURBS surface generation method.");
				break;
			case 3:
				SetMessage("You chose approximated NURBS surface generation method.");
				break;
			case 4:
				SetMessage("No supported");
				break;
			case 5:
				SetMessage("You chose NURBS surface generation method from control points.");
				break;
		}
