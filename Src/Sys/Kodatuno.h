...
#define KODATUNO_MAJOR_VER  3  				// kodatunoのメジャーバージョン
#define KODATUNO_MINOR_VER  0  				// kodatunoのマイナーバージョン
...
#define KDT_MOUSE_PRESS		1000	//マウスボタンプレス
#define KDT_MOUSE_RELEASE   1001	//マウスボタンリリース
#define KDT_MOUSE_LEFT      1002	//マウス左ボタン
#define KDT_MOUSE_MIDDLE    1003	//マウス中ボタン
#define KDT_MOUSE_RIGHT		1004	//マウス右ボタン
#define KDT_MOUSE_NONE		1005	//どのボタンでもない

#define KDT_MOD_CTRL	2001	//Ctrl修飾
#define KDT_MOD_SHIFT	2001	//Shift修飾
#define KDT_MOD_ALT		2002	//Alt修飾
#define KDT_MOD_NONE	2003	//どの修飾もない
...
// KODatUNOクラスを定義
class KODatUNO : public Describe_BODY		// BODY関連関数を集めたクラスを継承
{
// public関数の定義
public:
    void InitializeWin(int, int);			// Kodatunoの初期化
	void DeleteWin();						// Kodatunoの終了処理
	int OpenFile();							// ファイルオープン
	int OpenFile(char *);					// ファイルオープン(オーバーロード)
    std::string OpenInpFile();              // インプットファイルオープン
    ...
    int GetUpperConsoleHistory(std::string *);	// コマンド履歴を取得
    int GetLowerConsoleHistory(std::string *);	// コマンド履歴を取得
    void SetConsoleHistory(std::string);		// コマンド履歴を登録
    int GetConsoleHistoryNum();             	// コマンド履歴数を取得 <<< 追加
    void GetResizeWindow(int, int);					// ウィンドウがリサイズされたときに、そのウインドウサイズを読み込む
	void MouseEvent(long,long,short,short);	// マウスイベントの処理
    void MouseMotionEvent(long,long,short,short);// マウス移動中のイベント処理
    void MouseWheelEvent(long);				// マウスホイールの処理
    void ChangeViewX();						// X軸上の視点に切り替える
	void ChangeViewY();						// Y軸上の視点に切り替える
	void ChangeViewZ();						// Z軸上の視点に切り替える
	void ChangeViewXYZ();					// 鳥瞰視点に切り替える
    bool ChangeViewUp();					// 上矢印キーが押された場合のBODY回転
    bool ChangeViewDown();				// 下矢印キーが押された場合のBODY回転
    bool ChangeViewLeft();				// 左矢印キーが押された場合のBODY回転
    bool ChangeViewRight();				// 右矢印キーが押された場合のBODY回転
    ...
    void GetUserStat(int *,double []);      // UserStatusを取得
	...
	// Brand New!
	void DispUVdirection();					// 選択されている曲面のU,V方向を表示する
...
// Brand New!
void CmdUVdir(int,char *[]);			// コマンド(U,Vの方向を描画)
