	...
	while(init_allpt_flag == KOD_FALSE){
		...
		while(loop_count < ans_size){
			...
			for(int i=0;i<init_pt_num;i++){
				...
				if(int asdf = CheckClossedPoints(nurb,oldp,newp,init_pt[i]) == KOD_TRUE){
...
// 4次のルンゲクッタ法により交点を導出(オフセットNURBS曲面と平面)
// du(s)/ds = g(u,v),   dv(s)/ds = h(u,v)
// u(s+delta) = u(s) + (p1+2p2+2p3+p4)/6
// v(s+delta) = v(s) + (q1+2q2+2q3+q4)/6
// p1 = delta*g(u,v),   q1 = delta*h(u,v)
...
// 引数    *S1:面1,   *S2:面2,    *S_:連結後の面を格納
// 返値    成功：KOD_TRUE,  失敗：KOD_FALSE
...
// 返値　　KOD_TRUE：収束した    KOD_FALSE:収束しなかった
int NURBS_Func::CalcIntersecPtNurbsPt(NURBSS *S,Coord P,int Divnum,double Stepsize,Coord *Q)
{
	...
	for(int i=0;i<Divnum;i++){
		for(int j=0;j<Divnum;j++){
			...
			// 収束計算
			while(loopcount < LOOPCOUNTMAX/Stepsize){
...
// 外周トリム面内の点のみ残す
// 引数   *Trm:トリム面へのポインタ    *Pt:判別対象の(u,v)群      N:(u,v)群の数
// 返値   残った点の数　(外周トリムが存在しない：KOD_FALSE)
...
// 内周トリム面外の点のみ残す
// 引数   *Trm:トリム面へのポインタ    *Pt:判別対象の(u,v)群      N:(u,v)群の数
// 返値   残った点の数　(内周トリムが存在しない：KOD_FALSE)
...
// 引数    *C：NURBS曲線，   L：指定距離，    Init_t：解探索の初期パラメータ
...
// 同一点を除去する
// 引数   *S:曲面 *Q:曲面上の(u,v)パラメータ群(変更後の点群もここに格納される)   N:点数
// 返値   変更後の点数
...
// 引数  *C0：分割するNURBS曲線へのポインタ        *C1：分割されたNURBS曲線へのポインタ
//       *C2：分割されたNURBS曲線へのポインタ     L：端からの弧長
int NURBS_Func::DivNurbsC(NURBSC *C0, NURBSC *C1, NURBSC *C2, double L)
{
	double t = CalcParamLengthOnNurbsC(C0,L,0);		// 分割点のパラメータ値を取得

	DivNurbsCParam(C0,C1,C2,t);		// 分割
...
// 引数  *C0：分割するNURBS曲線へのポインタ        *C1：分割されたNURBS曲線へのポインタ
//       *C2：分割されたNURBS曲線へのポインタ     t：分割位置を表す曲線パラメータ
...
//  2本の曲線を繋げたときのコントロールポイントとウェイトを設定する
...
// NURBS曲線に新たなノットを挿入する
// 引数   *C：元のNURBS曲線  *C_：挿入対象のNURBS曲線     t：挿入するノット     deg：多重度
// 返値   k：新たなノットベクトル列におけるtの挿入位置
