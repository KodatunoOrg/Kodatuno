...
public:
	...
	double CalcMeanCurvature(SFQuant);							// オーバーロード
	...
	int GetPtsOnOuterTRMSurf(TRMS *,Coord *,int);					// 外周トリム面内の点のみ残す  (R3.0)
	int GetPtsOnInnerTRMSurf(TRMS *,Coord *,int);					// 内周トリム面外の点のみ残す  (R3.0)
	int GetPtsOnInnerOuterTRMSurf(TRMS *,Coord *,int);				// 内外周トリム面内の点のみ残す  (R3.0)
	...
	// Brand New!
	Coord CalcTanVecOnNurbsC(NURBSC *,double);					// NURBS曲線上のtにおける単位接ベクトルをもとめる
	int ConnectNurbsSU(NURBSS *,NURBSS *,NURBSS *);				// 2枚のNURBS曲面を連結する(U方向に長くなる)(S1_U1とS2_U0を連結)
	int ConnectNurbsSV(NURBSS *,NURBSS *,NURBSS *);				// 2枚のNURBS曲面を連結する(V方向に長くなる)(S1_V1とS2_V0を連結)
	double CalcCurvatureNurbsC(NURBSC *,double);				// NURBS曲線の曲率を求める
//	double CalcTorsionNurbsC(NURBSC *,double);					// NURBS曲線の捩率を求める
	int DivNurbsCParam(NURBSC *, NURBSC *, NURBSC *, double);	// NURBS曲線を指定したパラメータ値で分割する
	int DivNurbsC(NURBSC *, NURBSC *, NURBSC *, double);		// NURBS曲線を指定した位置（端からの距離）で分割する
	int ConnectNurbsC(NURBSC *, NURBSC *, NURBSC *);			// NURBS曲線の連結
	void ReverseNurbsC(NURBSC *);								// NURBS曲線のノットベクトル向きを反転する
	double CalcParamLengthOnNurbsC(NURBSC *,double,double);		// NURBS曲線において一端からの指定距離におけるパラメータ値を返す
...
private:
	...
	int SearchIntersectPt_OS(NURBSS *,Coord,Coord,double,double *,double *,int);	// 4次のルンゲクッタ法により交点を収束させる(オフセットNURBS曲面と平面)	
	...
	int ApproxTrimBorder(COMPC *,Coord *);							// トリム境界線を点群で近似する
	...
	Coord CalcIntersecPtsPlaneSearch_Sub(NURBSS *,double,double,Coord,Coord);	// 面から飛び出した(u,v)を参考に面のエッジ部(new_u,new_v)を得る
	...
	// Brand New!
	void SetKnotVecSU_ConnectS(NURBSS *,NURBSS *,NURBSS *);			// NURBS曲面連結用SUB関数(連結後の曲面のU方向ノット定義域を設定する)
	void SetKnotVecSV_ConnectS(NURBSS *,NURBSS *,NURBSS *);			// NURBS曲面連結用SUB関数(連結後の曲面のV方向ノット定義域を設定する)
	void SetCPSU_ConnectS(NURBSS *,NURBSS *,NURBSS *);				// NURBS曲面連結用SUB関数(連結後の曲面のU方向コントロールポイントとウェイトを設定する)
	void SetCPSV_ConnectS(NURBSS *,NURBSS *,NURBSS *);				// NURBS曲面連結用SUB関数(連結後の曲面のV方向コントロールポイントとウェイトを設定する)
	int InsertNewKnotOnNurbsC(NURBSC *,NURBSC *,double,int);		// NURBS曲線に新たなノットを挿入する
	void SetKnotVecC_ConnectC(NURBSC *,NURBSC *,NURBSC *);			// NURBS曲線連結用SUB関数(連結後の曲線のノット定義域を設定する)
	void SetCPC_ConnectC(NURBSC *,NURBSC *,NURBSC *);				// NURBS曲線連結用SUB関数(連結後の曲線のコントロールポイントとウェイトを設定する)
