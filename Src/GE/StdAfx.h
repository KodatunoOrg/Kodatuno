...
#define KOD_LARGE	0				// a > b　のシンボル
#define KOD_SMALL	1				// a < b　のシンボル
#define KOD_EQUAL	2				// a = b　のシンボル
#define KOD_LARGE_EQ 3				// a >= b　のシンボル
#define KOD_SMALL_EQ 4				// a <= b　のシンボル
#define LOW_LOW_ACCURACY -1			// 低低精度のシンボル
...
#define APPROX_ZERO_L_L 1.0e-3		// ゼロと見なせる値(低低精度)
#define APPROX_ZERO_L 1.0e-6		// ゼロと見なせる値(低精度)
#define APPROX_ZERO	1.0e-9			// ゼロと見なせる値(普通精度)
...
double CalcAnglePlaneVec(Coord,Coord);			// 平面と直線とのなす角を求める
...
Coord CalcNormVecFrom3Pts(Coord,Coord,Coord);	// 空間上の3点からなる平面の法線ベクトルを求める
...
double CalcOuterProduct2D(Coord,Coord);			// 外積を求める (2D Ver.)
double ClacPolygonArea2D(Coord [],int);			// 2D平面上の多角形の符号付き面積を得る
int DiscriminateCW2D(Coord [],int);				// 2D平面上の多角形が時計回りか反時計回りかを判別する
...
void AddMxMx(Matrix,Matrix,Matrix,int,int);		// 行列同士の足し算
...
void SetColorStat(DispStat *ds,float r, float g, float b, float a=0.5);	// カラーステータスを変更
...
void Reverse(Coord [],int);				// Coord配列の反転操作
void Reverse(double [],int);			// double配列の反転操作
int CatCoord(Coord [],Coord [],int,int,int);	// ある配列の後ろに新たな配列を繋げる
...
void CoordToArray(Coord,double []);		// Coordをdouble配列に代入
void CoordToArray2D(Coord,double []);	// Coordをdouble配列に代入
