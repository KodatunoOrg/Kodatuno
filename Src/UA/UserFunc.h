...
public:
    ...
    UserStat_ *GetUserStatus();                                  // UserStatステータスを取得
	void ExecdClear();											// Canncelボタンが押されるとこの関数が実行されるので、ここに初期化コードを挿入しておく
	int ExecWidgetFunc(int);									// 各Userボタンに対してWS関数を使用したい場合はこの関数内で割り当てること
...
private:
    ...
    char MenuLabel6[LABELNAMEMAX];				// User関数6のメニュー名
    char MenuLabel7[LABELNAMEMAX];				// User関数7のメニュー名
    char MenuLabel8[LABELNAMEMAX];				// User関数8のメニュー名
